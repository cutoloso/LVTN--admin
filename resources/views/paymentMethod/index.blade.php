...
@section('header.title','Phương thức thanh toán')
@section('body.title','Danh sách phương thức thanh toán')
        ...
        <div class="card-body">
            <div class="table-responsive">
                ...
                <table class="table table-hover table-bordered" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                    @foreach ($payment_method as $pay_mth)
                        <tr>
                            <td>{{ $pay_mth->name }}</td>
                            <td>{{ $pay_mth->description }}</td>
                            <td>{{ $pay_mth->updated_at }}</td>
                            <td class="text-center">
                                <button class="btn btn-secondary js-btn-edit" data-id="{{$pay_mth->id}}" data-token="{{ csrf_token() }}"><i class="fas fa-edit"></i></button>
                            </td>
                            <td class="text-center">
                                <button class="btn btn-danger js-btn-del" data-id="{{$pay_mth->id}}" data-token="{{ csrf_token() }}"><i class="fas fa-minus"></i></button>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h4 class="modal-title">Thêm mới phương thức thanh toán</h4>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>

                <form action="{{route('payment-method.store')}}" name="frmAdd" method="post" enctype="multipart/form-data">
                    <!-- Modal body -->
    ...
    <div class="modal fade" id="modalEdit">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <!-- Modal Header -->
                <div class="modal-header">
                    <h4 class="modal-title">Chỉnh sửa phương thức thanh toán</h4>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
        ...
        $(document).ready(function(){
            ...
            $('.js-btn-del').click(function () {
                if(confirm('Vui lòng xác nhận trước khi xóa!')){
                    let id = $(this).data('id');
                    $.ajax({
                        url: '{{route('payment-method.index')}}'+'/'+id,
                        type: 'DELETE',
                        data: {
                            "_token": "{{ csrf_token() }}"
                        },
            ...
            $('.js-btn-edit').click(function () {
                let id = $(this).data('id');
                let url = '{{route('payment-method.index')}}' + '/' + id;
                $.ajax({
                    url: url,
                    type: 'GET',
                    data: {
